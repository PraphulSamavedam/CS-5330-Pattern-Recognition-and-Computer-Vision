...
# Local imports
from models import BaseNetwork, ManyParallelFiltersNetWork
from utils import (get_fashion_mnist_data_loaders, quick_train_and_analyse_model, 
...
def experiment_with_multiple_parallel_filters(train_data_loader:DataLoader,
    ...
    """
    This function experiments by varying the number of filters
    being used at convolution layer.
    """
    train_losses = []
    test_losses = []
    train_accuracies = []
    test_accuracies = []
    filter_counts = [x for x in range(2, 31, 2)]
    time_elapsed = []
    for filter_count in filter_counts:
        ...
        (train_loss, test_loss,
         train_acc, test_acc,
         time_elapse) = quick_train_and_analyse_model(model,
                                                      train_data_loader,
                                                      test_data_loader,optimizer,
        ...
        time_elapsed.append(time_elapse)
   
    plt.figure(figsize=(10, 10))
    plt.title("Exp Vary Filter Count Train and Test Accuracy")
    plt.scatter(filter_counts,train_accuracies, color='b', label="Train Accuracy", marker='*')
    plt.plot(filter_counts, train_accuracies, color='b')
    plt.scatter(filter_counts, test_accuracies, color='r', label="Test Accuracy", marker="+")
    plt.savefig("Exp Vary Filter Count Train and Test Accuracy.png")
    plt.clf()

    plt.figure(figsize=(10, 10))
    plt.title("Exp Vary Filter Count Train and Test Loss")
    ...
    plt.figure(figsize=(10, 10))
    plt.title("Exp Vary Filter Count Train Time Elapses")
    plt.plot(filter_counts, time_elapsed, color='b', label="Time elapse")
    plt.savefig("Exp Vary Filter Count Training time.png")
    plt.clf()
...
def main():
    ...
    neural_net = BaseNetwork
    epoch_count = 20
    experiment_with_epochs(neural_net, train_data_loader, test_data_loader, learning_rate,momentum,
