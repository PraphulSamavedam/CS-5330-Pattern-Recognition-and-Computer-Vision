...
def visualize_filters(n_layer_weights: torch.Tensor, layer_number:int = 0) -> None:
    ...
    plt.title(f"Visualizing filters learned in {layer_number} layer")
    plt.xticks = []
    plt.yticks = []
    plt.axis("off")
...
def filter_understanding(n_layer_weights: torch.Tensor, image: torch.Tensor) -> None:
    """This function applies the different filters on the image_provided and outputs the results
    Returns 
    None
    Params
    ...
    plt.title("Understanding the filters")
    plt.axis("off")
    plt.xticks = []
    plt.yticks = []
    with torch.no_grad():
        for fltr in range(1, filter_count, 1):
            plt.subplot(rows, columns, (fltr-1)*2 + 1)
            plt.axis("off")
            plt.imshow(n_layer_weights[fltr-1, 0].detach().squeeze(), cmap='viridis')
            kernel = n_layer_weights[fltr,0].detach().numpy()
            source = image.detach().numpy()
            filtered_image = cv2.filter2D(src=source, ddepth = -1, kernel=kernel)
