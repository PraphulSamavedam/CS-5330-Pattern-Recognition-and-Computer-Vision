...
import argparse
import torch
from torch.nn import functional as F # For parsing the argument passed to file as script.
...
def parse_arguments(description:str):
    ...
    parser = argparse.ArgumentParser(prog=f"{__file__}",
                                     ...
                                     description=f"{description}",epilog="")
    ...
    return samples, learning_rate, momentum, log_interval, train_bsize, test_bsize, epochs


def train_network(model:nn.Module, train_data_loader: DataLoader, optimizer: optim,
...
def train_and_plot_accuracies(model: nn.Module, epochs: int, optimizer: optim,
                              train_data_loader: DataLoader, log_interval:int,
                              batch_size: int,
    ...
    for epoch in range(1, epochs+1, 1):
        losses, indices = train_network(model= model, train_data_loader= train_data_loader,
                    optimizer=optimizer, log_interval=log_interval,
                    epoch=epoch, batch_size=batch_size,
                    model_path=model_path, optim_path=optimizer_path)
...
def visualize_errors_over_training(train_idx, train_errors, test_idx, test_errors) -> None:
