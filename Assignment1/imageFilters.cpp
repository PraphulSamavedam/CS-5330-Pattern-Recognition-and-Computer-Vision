...
int sobelY3x3(cv::Mat &src, cv::Mat &dst) {
	...
	*/
	...
	for (int row = 0; row < src.rows; row++)
	{
		// Destination for this single operation is intermediate Mat
		cv::Vec3b* srcPtr = src.ptr<cv::Vec3b>(row);
	...
	*/
	for (int row = 1; row < src.rows - 1; row++)
	{
		// Source for this single operation is intermediate as it has the convoluted values
		cv::Vec3s* rowM1Ptr = intermediate.ptr<cv::Vec3s>(row - 1);
		cv::Vec3s* rowP1Ptr = intermediate.ptr<cv::Vec3s>(row + 1);
		cv::Vec3s* dstPtr = dst.ptr<cv::Vec3s>(row);
		for (int col = 0; col < src.cols; col++)
...
int cartoon(cv::Mat& src, cv::Mat& dst, int levels, int magThreshold) {
	...
	cv::imshow("Cartoon Magnitude Image", gradient_magnitude_image);
	...
	cv::imshow("Cartoon BQ Image", blur_quantized_image);
	...
	//cv::imshow("Cartoon BQ  Alpha Image", blur_quantized_image);
	...
	cv::imshow("Scaled SobelX image", scaled_sx_image);

	cv::Mat scaled_sy_image;
	cv::convertScaleAbs(sy_image, scaled_sy_image);
	cv::imshow("Scaled SobelY image", scaled_sy_image);

	cv::Mat scaled_magnitude_image;
	cv::convertScaleAbs(gradient_image, scaled_magnitude_image);
	cv::imshow("Scaled Magnitude Image", scaled_magnitude_image);
	*/
	return 0;
}
