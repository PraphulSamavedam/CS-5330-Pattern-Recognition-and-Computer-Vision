...
* @note:
	...
	if 'S' is pressed: Sepia effect video feed is toggled.
	...
	if 'c' is pressed: Cartoon vidoe feed is toggled
	if 'e' is pressed: Extreme blur video feed is toggled.
	*/
...
int main(int argc, char* argv[]) {
	...
	cv::Mat sepiaImg; // Will be initialized when effect is enabled.

	bool strongBlurEnabled = false;
	cv::Mat strongBlurImg; // Will be initialized when effect is enabled.
	...
	while (true)
	{
		*capture >> frame; 
		...
		if (sepiaEnabled) {
			sepiaEffect(frame, sepiaImg);
			cv::namedWindow("Sepia effect Video", windowSize);
			cv::imshow("Sepia effect Video", sepiaImg);
		}
		else if (cv::getWindowProperty("Sepia effect Video", cv::WND_PROP_VISIBLE) == 1.0) {
			cv::destroyWindow("Sepia effect Video");
		}

		if (strongBlurEnabled) {
			strongBlur(frame, strongBlurImg);
			for (int i = 0; i < 10; i++)
			{
				strongBlur(strongBlurImg, strongBlurImg);
			}
			cv::namedWindow("Strongly blurred Video", windowSize);
			cv::imshow("Strongly blurred Video", strongBlurImg);
		}
		else if (cv::getWindowProperty("Strongly blurred Video", cv::WND_PROP_VISIBLE) == 1.0) {
			cv::destroyWindow("Strongly blurred Video");
		}
		...
		{	//Saves the image 
			...
			saveImage(sepiaImg, "Sepia");
			saveImage(strongBlurImg, "StrongBlur");
		}
		...
		else if (key == 'S') { // Sepia Effect
			sepiaEnabled = not(sepiaEnabled);
			sepiaImg = cv::Mat(frame.size(), frame.type()); // To ensure initialization.
			continue;
		}
		else if (key == 'w') { // Strongly blurs the video feed
			strongBlurEnabled = not(strongBlurEnabled);
			strongBlurImg = cv::Mat(frame.size(), frame.type()); // To ensure initialization.
			continue;
		}
	}
