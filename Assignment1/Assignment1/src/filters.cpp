...
/**
* This function adds Sepia filter to the image
*
* r = min(255, 0.189B + 0.769G + 0.393R)
* g = min(255, 0.168B + 0.686G + 0.349R)
* b = min(255, 0.131B + 0.534G + 0.272R)
*  
* @param src is the image which needs to have sepia
* @param dst is the destination image which has sepia filter on the src image.
* @return 0 if the operation is successful.
*		 -100 if the src and dst are not of same size.
*
*/
int sepiaEffect(cv::Mat& src, cv::Mat& dst) {
	for (int row = 0; row < src.rows; row++)
	{
		cv::Vec3b* srcRowPtr = src.ptr<cv::Vec3b>(row);
		cv::Vec3b* dstRowPtr = dst.ptr<cv::Vec3b>(row);
		for (int col = 0; col < src.cols; col++)
		{
			short tr = srcRowPtr[col][0] * 0.189 + srcRowPtr[col][1] * 0.769 + srcRowPtr[col][2] * 0.393;
			short tg = srcRowPtr[col][0] * 0.168 + srcRowPtr[col][1] * 0.686 + srcRowPtr[col][2] * 0.349;
			short tb = srcRowPtr[col][0] * 0.131 + srcRowPtr[col][1] * 0.534 + srcRowPtr[col][2] * 0.272;
			dstRowPtr[col][0] = tb > 255 ? 255 : tb;
			dstRowPtr[col][1] = tg > 255 ? 255 : tg;
			dstRowPtr[col][2] = tr > 255 ? 255 : tr;
		}
	}
	return 0;
}